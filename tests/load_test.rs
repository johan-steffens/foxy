...
impl LoadTestResults {
    pub fn print_summary(&self) {
        ...
        println!("Avg Response Time: {:.2}ms", self.avg_response_time.as_millis());
        println!("Min Response Time: {:.2}ms", self.min_response_time.as_millis());
        println!("Max Response Time: {:.2}ms", self.max_response_time.as_millis());
        println!("95th Percentile: {:.2}ms", self.p95_response_time.as_millis());
        println!("99th Percentile: {:.2}ms", self.p99_response_time.as_millis());
    }
}
...
async fn test_basic_load() {
    ...
    // Assertions
    assert!(results.successful_requests > 0, "Should have some successful requests");
    assert!(results.requests_per_second > 0.0, "Should have positive RPS");
    assert!(results.avg_response_time < Duration::from_secs(1), "Average response time should be reasonable");
    ...
    assert!(success_rate > 0.9, "Success rate should be > 90%, got {:.2}%", success_rate * 100.0);
...
async fn test_high_concurrency_load() {
    ...
    assert!(results.successful_requests > 0, "Should handle concurrent requests");
    assert!(results.requests_per_second > 10.0, "Should maintain reasonable throughput");
    
    // Allow for some failures under high load
    let success_rate = results.successful_requests as f64 / results.total_requests as f64;
    assert!(success_rate > 0.8, "Success rate should be > 80% under high load, got {:.2}%", success_rate * 100.0);
...
async fn test_sustained_load() {
    ...
    assert!(results.successful_requests > 0, "Should handle sustained load");
    assert!(results.requests_per_second > 5.0, "Should maintain throughput over time");
    
    // Check that response times don't degrade too much
    assert!(results.p95_response_time < Duration::from_secs(2), 
        "95th percentile should be reasonable: {:?}", results.p95_response_time);
