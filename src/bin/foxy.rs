...
use foxy::{Foxy, info_fmt};
#[cfg(feature = "opentelemetry")]
use foxy::{error_fmt, warn_fmt};
use std::env;
use std::error::Error;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if let Some(ref path) = file_from_env {
        ...
        println!(
            "No FOXY_CONFIG_FILE env var found. Attempting to use default configuration path: {fallback_path}"
        );
    ...
    #[cfg(feature = "opentelemetry")]
    {
        match proxy
            .config()
            .get::<OpenTelemetryConfig>("proxy.opentelemetry")
        {
            Ok(Some(otel_config)) => {
                if !otel_config.endpoint.is_empty() {
                    ...
                    warn_fmt!(
                        "Startup",
                        "OpenTelemetry endpoint is not configured. Skipping initialization."
                    );
                }
            }
            Ok(None) => {
            ...
            Err(e) => {
                info_fmt!(
                    "Startup",
                    "Failed to read OpenTelemetry configuration: {}",
                    e
                );
            }
        }
